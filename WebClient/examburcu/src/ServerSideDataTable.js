...
import { getDataByParams } from './api-client';
import '../src/style/datatable.css'; // Harici CSS dosyasını import et
...
function ServerSideDataTable({ controller, action = '', title, tableName, params = {}, onAdd, onEdit, onDelete, onSoftDelete, onExportExcel }) {
  ...
  const fetchData = useCallback(async (abortSignal) => {
    ...
    try {
      const result = await getDataByParams(apiUrl, finalParams, { signal: abortSignal });
      // GENİŞLETİLMİŞ DÜZELTME: En yaygın isimleri (Data, Records) ekleyin ve DİREKT DİZİYİ de kabul edin.
        const items = result.Items || result.items || result.Data || result.data || result.Records || result.records || (Array.isArray(result) ? result : []);
        
        const receivedTotalPages = result.TotalPages || result.totalPages || result.PageCount || result.pageCount || 1;
      setData(items);
      ...
      if (items.length > 0) {
            setColumns(prevColumns => {
                ...
                return prevColumns; // Eğer sütunlar zaten varsa, state'i değiştirmeyin!
            });
        }
    } catch (e) {
  ...
  }, [apiUrl, currentPage, paramsString, actionColumn]);
